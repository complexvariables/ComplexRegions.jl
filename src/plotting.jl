...
Makie.convert_single_argument(z::AbstractVector{<:Complex}) = map(z_to_point, z)

# Convert a pathlike object to a vector of points
curve_to_points(c::AbstractCurveOrPath) = z_to_point.(plotdata(c))
Makie.convert_single_argument(c::AbstractCurveOrPath) = curve_to_points(c)

# Convert a generic region to a Makie Polygon
function Makie.convert_single_argument(R::ConnectedRegion{N}) where N
    outer = curve_to_points(outerboundary(R))
    inner = curve_to_points.(innerboundary(R))
    return GB.Polygon(outer, inner)
end

# Conversions for particular cases
function Makie.convert_single_argument(R::InteriorSimplyConnectedRegion)
    ∂R = curve_to_points(boundary(R))
    return GB.Polygon(∂R)
end

function Makie.convert_single_argument(R::ExteriorRegion{N}) where N
    return Makie.convert_single_argument(truncate(R))
end
...
Makie.convert_single_argument(A::Annulus) = ConnectedRegion{2}(A.outer, [A.inner])
